(() => {
  ...
  function renderConstructorsStandings() {
    ...
    const rows = sortConstructorsTeams(data.teams).map((team) => '<tr><td class="standing-rank ' + (team.position <= 3 ? 'top-three' : '') + '">' + String(team.position).padStart(2, '0') + '</td><td><button class="constructors-team-link" type="button" data-constructor-team="' + escapeHtml(team.id) + '">' + escapeHtml(team.name) + '</button></td><td class="record-total">' + constructorsPoints(team.seasonPoints) + '</td><td>' + (team.position === 1 || team.gap < 0.0000001 ? '—' : '-' + constructorsPoints(team.gap)) + '</td><td>' + average(team.avgFinish) + '</td><td>' + average(team.avgQualifying) + '</td><td>' + (team.lapsLed || '—') + '</td><td>' + (team.wins || '—') + '</td><td>' + (team.podiums || '—') + '</td><td>' + (team.poles || '—') + '</td><td>' + (team.fastestLaps || '—') + '</td></tr>').join('');
