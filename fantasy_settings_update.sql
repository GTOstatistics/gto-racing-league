...
-- security-definer functions intentionally use a restricted search path, so
-- this explicit schema reference keeps token and admin-code hashing working.
create or replace function public.fantasy_hash(value text)
returns text language plpgsql immutable as $$
declare hashed bytea;
begin
  begin
    execute 'select extensions.digest(convert_to($1, ''UTF8''), ''sha256''::text)'
      into hashed using value;
  exception when undefined_function or invalid_schema_name then
    execute 'select public.digest(convert_to($1, ''UTF8''), ''sha256''::text)'
      into hashed using value;
  end;
