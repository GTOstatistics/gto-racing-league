(() => {
  ...
  function renderTeamRecords(season) {
    const careerRows = teamSeasonData(season).teams.map((team) => ({ ...teamCombinedStats(team.id, 'career'), name: team.name })); const seasonRows = teamSeasonOptions().flatMap((item) => teamSeasonData(item).teams.map((team) => ({ ...team, name: team.name, season: item.name, consecutiveWins: teamLongestSeasonStreak(team, item, (metric) => metric.entries.some((entry) => entry.result.position === 1)), consecutivePodiums: teamLongestSeasonStreak(team, item, (metric) => metric.entries.some((entry) => entry.result.position <= 3)) }))); const raceRows = teamSeasonOptions().flatMap((item) => { const standings = teamSeasonData(item).teams; return getArchiveRounds(item).flatMap((round) => teamRoundRankings(item, round).map((row, index, all) => ({ name: row.team.name, team: row.team, season: item.name, round: round.index + 1, lapsLed: row.lapsLed, avgFinish: row.avgFinish, topFive: row.entries.filter((entry) => entry.result.position <= 5).length, topTen: row.entries.filter((entry) => entry.result.position <= 10).length, margin: index && all[0] ? row.avgFinish - all[0].avgFinish : null }))); });
