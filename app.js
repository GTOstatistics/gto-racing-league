(() => {
  ...
  function constructorsSeasonStandings(season) {
    ...
    // Individual standings remain season totals, while Constructors points are
    // intentionally scored race by race: average the active team drivers in
    // each completed round, then add those round averages across the season.
    ...
    const teams = definition.teams.map((team) => {
      ...
      const roundAverages = rounds.map(({ race, index }) => {
        const starters = members.map((member) => {
          const result = member.driver?.results[index];
          if (!result || result.position === null || result.position === undefined) return null;
          const bonuses = getChampionshipBonus({ ...result, race, roundIndex: index }, season);
          return { name: member.name, points: (result.points || 0) + bonuses.totalBonusPoints };
        }).filter(Boolean);
        const totalPoints = starters.reduce((total, entry) => total + entry.points, 0);
        return {
          race,
          index,
          starters,
          totalPoints,
          averagePoints: starters.length ? totalPoints / starters.length : 0
        };
      });
      const scoringRounds = roundAverages.filter((round) => round.starters.length);
      const seasonPoints = scoringRounds.reduce((total, round) => total + round.averagePoints, 0);
      return {
        ...team,
        members,
        active,
        ...
        bestFinish: stats.completed.length ? Math.min(...stats.completed.map((entry) => entry.position)) : null,
        bestQualifying: stats.qualifying.length ? Math.min(...stats.qualifying.map((entry) => entry.qualifyingPosition)) : null,
        totalDriverPoints: active.reduce((total, member) => total + (member.driver?.points || 0), 0),
        averagePoints: scoringRounds.length ? seasonPoints / scoringRounds.length : null,
        roundAverages,
        scoringRounds,
        seasonPoints
      };
    }).sort((first, second) => second.seasonPoints - first.seasonPoints || first.name.localeCompare(second.name));
    const leaderPoints = teams[0]?.seasonPoints || 0;
    return {
      definition,
  ...
  function renderConstructorsStandings() {
    ...
    const rows = sortConstructorsTeams(data.teams).map((team) => '<tr><td class="standing-rank ' + (team.position <= 3 ? 'top-three' : '') + '">' + String(team.position).padStart(2, '0') + '</td><td>' + teamLink(team, 'constructors-team-link') + '</td><td class="record-total">' + constructorsPoints(team.seasonPoints) + '</td><td>' + (team.position === 1 || team.gap < 0.0000001 ? '—' : '-' + constructorsPoints(team.gap)) + '</td><td>' + average(team.avgFinish) + '</td><td>' + average(team.avgQualifying) + '</td><td>' + (team.lapsLed || '—') + '</td><td>' + (team.wins || '—') + '</td><td>' + (team.podiums || '—') + '</td><td>' + (team.poles || '—') + '</td><td>' + (team.fastestLaps || '—') + '</td></tr>').join('');
    container.innerHTML = '<div class="constructors-heading"><div><p class="eyebrow">' + (data.definition.kind === 'hypothetical' ? 'Retrospective constructors championship' : 'Official constructors championship') + '</p><h3>Constructors Standings</h3></div><p>' + subtitle + ' Team statistics combine all classified starts by active roster drivers.</p></div><div class="table-shell constructors-table-shell"><table class="profile-table constructors-table"><thead><tr>' + constructorSortHeader('Position', 'position') + constructorSortHeader('Team', 'name') + constructorSortHeader('Constructors points', 'seasonPoints') + constructorSortHeader('Gap', 'gap') + constructorSortHeader('Avg. finish', 'avgFinish') + constructorSortHeader('Avg. qualifying', 'avgQualifying') + constructorSortHeader('Laps led', 'lapsLed') + constructorSortHeader('Wins', 'wins') + constructorSortHeader('Podiums', 'podiums') + constructorSortHeader('Pole positions', 'poles') + constructorSortHeader('Fastest laps', 'fastestLaps') + '</tr></thead><tbody>' + rows + '</tbody></table></div>' + renderConstructorProfile(selected, data.definition);
  }
  ...
  function enhProfileTabs() {
    return '<div class="profile-data-tabs" role="tablist" aria-label="Driver profile views">' +
      [['overview', 'Overview'], ['tracks', 'Tracks'], ['team', 'Team'], ['charts', 'Charts']].map((item) => '<button type="button" role="tab" data-profile-tab="' + item[0] + '" aria-selected="' + (state.profileTab === item[0]) + '">' + item[1] + '</button>').join('') + '</div>';
  }
  ...
  function renderDriverProfile() {
    ...
    const hero = '<article class="profile-hero"><div><p class="eyebrow">Career at a glance</p><h3>' + escapeHtml(driver.name) + '</h3><p>' + driver.seasons.length + ' season' + (driver.seasons.length === 1 ? '' : 's') + ' · ' + driver.completed.length + ' race start' + (driver.completed.length === 1 ? '' : 's') + ' · ' + number.format(driver.points) + ' career points</p></div><div class="profile-metrics"><div><strong>' + driver.wins + '</strong><span>Wins</span></div><div><strong>' + driver.podiums + '</strong><span>Podiums</span></div><div><strong>' + driver.poles + '</strong><span>Poles</span></div><div><strong>' + driver.fastestLaps + '</strong><span>Fastest laps</span></div><div><strong>' + driver.lapsLed + '</strong><span>Laps led</span></div></div></article>' + driverCurrentTeamCard(driver.name) + enhCareerBestsBox(bests) + enhProfileTabs();
    if (state.profileTab === 'tracks') { elements.driverProfile.innerHTML = hero + enhProfileTracks(driver); return; }
    if (state.profileTab === 'team') { elements.driverProfile.innerHTML = hero + renderDriverTeamStatistics(driver.name); return; }
  ...
  function predictionRaceSimulationData(season, round, includeCurrentSeason = true) {
    ...
    const finishingOrders = []; const poleWinners = []; const fastestLapWinners = [];
    ...
    for (let simulation = 0; simulation < simulations; simulation += 1) {
      ...
      const fastestLapWinner = predictionPick(all, sampledRows, random, 'fastestLapWeight');
      counts[fastestLapWinner].fastestLap += 1; fastestLapWinners.push(fastestLapWinner);
  ...
  function predictionHeadToHeadCell(name, probability, odds, status) {
    return '<div class="prediction-h2h-driver"><strong>' + driverLink(name, 'record-driver-link') + '</strong><span class="prediction-h2h-mobile-metrics">' + predictionPercent(probability) + ' <b aria-hidden="true">&middot;</b> ' + odds + '</span><small>' + status + '</small></div>';
  }
  function predictionH2HDefaultDirection(key) {
    return key === 'driver1Name' || key === 'driver2Name' || key === 'closeness' ? 'asc' : 'desc';
  }
  function predictionH2HValue(matchup, key) {
    if (key === 'driver1AmericanOdds') return matchup.driver1Probability;
    if (key === 'driver2AmericanOdds') return matchup.driver2Probability;
    return matchup[key];
  }
  function predictionSortHeadToHeadRows(rows, view) {
    const key = view === 'all' ? state.predictionH2HAllSortKey : state.predictionH2HDriverSortKey;
    const direction = view === 'all' ? state.predictionH2HAllSortDirection : state.predictionH2HDriverSortDirection;
    return rows.slice().sort((first, second) => {
      const firstValue = predictionH2HValue(first, key); const secondValue = predictionH2HValue(second, key);
      const comparison = typeof firstValue === 'string' ? firstValue.localeCompare(secondValue) : firstValue - secondValue;
      return comparison * (direction === 'asc' ? 1 : -1) || first.driver1Name.localeCompare(second.driver1Name) || first.driver2Name.localeCompare(second.driver2Name);
    });
  }
  function predictionAllMatchups(headToHead) {
    return predictionSortHeadToHeadRows(headToHead.matchups.map((matchup) => predictionOrientMatchup(matchup)), 'all');
  }
  function predictionDriverMatchups(headToHead, selectedDriver) {
    return predictionSortHeadToHeadRows(headToHead.matchups.filter((matchup) => matchup.driverAName === selectedDriver || matchup.driverBName === selectedDriver).map((matchup) => predictionOrientMatchup(matchup, selectedDriver)), 'driver');
  }
  ...
  function predictionHeadToHeadHeader(label, key, view) {
    const activeKey = view === 'all' ? state.predictionH2HAllSortKey : state.predictionH2HDriverSortKey;
    const direction = view === 'all' ? state.predictionH2HAllSortDirection : state.predictionH2HDriverSortDirection;
    const active = activeKey === key;
    return '<th><button class="sort-button" type="button" data-prediction-h2h-sort-key="' + key + '" aria-pressed="' + active + '">' + label + ' <span class="sort-icon" aria-hidden="true">' + (active ? (direction === 'asc' ? '↑' : '↓') : '↕') + '</span></button></th>';
  }
  function predictionHeadToHeadSortMenu(view) {
    const key = view === 'all' ? state.predictionH2HAllSortKey : state.predictionH2HDriverSortKey;
    const firstLabel = view === 'all' ? 'Driver 1' : 'Selected Driver'; const secondLabel = view === 'all' ? 'Driver 2' : 'Opponent';
    const option = (value, label) => '<option value="' + value + '"' + (key === value ? ' selected' : '') + '>' + label + '</option>';
    return '<label><span>Sort table</span><select data-prediction-h2h-sort-select><option value="closeness"' + (key === 'closeness' ? ' selected' : '') + '>Most Even</option>' + option('driver1Name', firstLabel + ' Name') + option('driver1Probability', firstLabel + ' Probability') + option('driver1AmericanOdds', firstLabel + ' American Odds') + option('driver2Name', secondLabel + ' Name') + option('driver2Probability', secondLabel + ' Probability') + option('driver2AmericanOdds', secondLabel + ' American Odds') + '</select></label>';
  }
  function predictionHeadToHeadTable(matchups, ariaLabel, view) {
    const firstLabel = view === 'all' ? 'Driver 1' : 'Selected Driver'; const secondLabel = view === 'all' ? 'Driver 2' : 'Opponent';
    const headers = predictionHeadToHeadHeader(firstLabel, 'driver1Name', view) + predictionHeadToHeadHeader(firstLabel + ' %', 'driver1Probability', view) + predictionHeadToHeadHeader(firstLabel + ' odds', 'driver1AmericanOdds', view) + '<th class="prediction-h2h-vs-cell" scope="col">VS</th>' + predictionHeadToHeadHeader(secondLabel, 'driver2Name', view) + predictionHeadToHeadHeader(secondLabel + ' %', 'driver2Probability', view) + predictionHeadToHeadHeader(secondLabel + ' odds', 'driver2AmericanOdds', view);
    const rows = matchups.map((matchup) => '<tr><td class="prediction-h2h-name-cell">' + predictionHeadToHeadCell(matchup.driver1Name, matchup.driver1Probability, matchup.driver1AmericanOdds, matchup.driver1Status) + '</td><td class="prediction-h2h-prob-cell">' + predictionPercent(matchup.driver1Probability) + '</td><td class="prediction-h2h-odds-cell">' + matchup.driver1AmericanOdds + '</td><td class="prediction-h2h-vs-cell" aria-label="versus">VS</td><td class="prediction-h2h-name-cell">' + predictionHeadToHeadCell(matchup.driver2Name, matchup.driver2Probability, matchup.driver2AmericanOdds, matchup.driver2Status) + '</td><td class="prediction-h2h-prob-cell">' + predictionPercent(matchup.driver2Probability) + '</td><td class="prediction-h2h-odds-cell">' + matchup.driver2AmericanOdds + '</td></tr>').join('');
    return '<div class="table-shell prediction-h2h-table-shell"><table class="profile-table prediction-h2h-table" aria-label="' + ariaLabel + '"><thead><tr>' + headers + '</tr></thead><tbody>' + (rows || '<tr><td colspan="7">No eligible matchups are available.</td></tr>') + '</tbody></table></div>';
  }
  function renderPredictionHeadToHead(season, round) {
    ...
    if (!names.includes(state.predictionH2HFilterDriver)) state.predictionH2HFilterDriver = '';
    ...
    if (view === 'all') {
      const filterDriver = state.predictionH2HFilterDriver || '';
      const all = predictionAllMatchups(headToHead); const filtered = all.filter((matchup) => !filterDriver || matchup.driver1Name === filterDriver || matchup.driver2Name === filterDriver);
      const visibleLimit = Math.max(25, state.predictionH2HVisible || 25); const visible = filtered.slice(0, visibleLimit);
      const toolbar = '<div class="prediction-h2h-toolbar"><label><span>Choose driver</span><select data-prediction-h2h-filter-driver><option value="">All drivers</option>' + names.map((name) => '<option value="' + escapeHtml(name) + '"' + (name === filterDriver ? ' selected' : '') + '>' + escapeHtml(name) + '</option>').join('') + '</select></label>' + predictionHeadToHeadSortMenu('all') + '</div>';
      ...
      return '<section class="prediction-panel prediction-h2h-panel"><div class="panel-title"><div><p class="eyebrow">Race forecast</p><h3>Head-to-Head Odds</h3></div><p>Every eligible driver pair is compared across the same 7,000 event-specific race simulations used for the full forecast. Click a column heading to sort.</p></div>' + viewControls + note + toolbar + count + predictionHeadToHeadTable(visible, 'All head-to-head matchups', 'all') + showMore + '</section>';
    }
    const selected = state.predictionH2HDriver; const driverRows = selected ? predictionDriverMatchups(headToHead, selected) : [];
    const toolbar = '<div class="prediction-h2h-toolbar prediction-h2h-driver-toolbar"><label><span>Choose driver</span><select data-prediction-h2h-driver><option value="">Choose driver</option>' + names.map((name) => '<option value="' + escapeHtml(name) + '"' + (name === selected ? ' selected' : '') + '>' + escapeHtml(name) + '</option>').join('') + '</select></label>' + predictionHeadToHeadSortMenu('driver') + '</div>';
    const body = selected ? predictionHeadToHeadSummary(selected, driverRows) + predictionHeadToHeadTable(driverRows, escapeHtml(selected) + ' head-to-head matchups', 'driver') : '<p class="no-profile">Choose a driver to compare their odds against every other eligible driver in this round.</p>';
    return '<section class="prediction-panel prediction-h2h-panel"><div class="panel-title"><div><p class="eyebrow">Race forecast</p><h3>Head-to-Head Odds</h3></div><p>Each matchup uses the selected round\'s track, car-category, recent-form, and career inputs. Click a column heading to sort.</p></div>' + viewControls + note + toolbar + body + '</section>';
  }
  function renderPredictions() {
    ...
    elements.predictionsContent.innerHTML = controls + '<section class="prediction-panel"><div class="panel-title"><div><p class="eyebrow">Live title outlook</p><h3>Season 5 championship predictions</h3></div><p>5,000 full-season simulations use current points and every remaining scheduled race. Each scheduled round uses its own track/car-specific field rating; pole and fastest-lap bonus points are modeled for unrecorded rounds.</p></div><div class="table-shell prediction-table-shell"><table class="profile-table prediction-table"><thead><tr>' + predictionHeader('Driver', 'name') + predictionHeader('Current points', 'currentPoints') + predictionHeader('Model rating', 'preseasonRating') + predictionHeader('Championship %', 'championshipProbability') + '<th>Title odds</th>' + predictionHeader('Top 3 %', 'topThreeProbability') + predictionHeader('Top 5 %', 'topFiveProbability') + '</tr></thead><tbody>' + championshipRows.map((row) => '<tr><td>' + driverLink(row.name, 'record-driver-link') + '</td><td>' + number.format(row.currentPoints) + '</td><td class="prediction-rating">' + row.preseasonRating.toFixed(1) + '</td><td>' + predictionPercent(row.championshipProbability) + '</td><td>' + predictionAmericanOdds(row.championshipProbability) + '</td><td>' + predictionPercent(row.topThreeProbability) + '</td><td>' + predictionPercent(row.topFiveProbability) + '</td></tr>').join('') + '</tbody></table></div></section>' + renderTeamChampionshipPredictions(season) + '<section class="prediction-panel preseason-panel"><div class="panel-title"><div><p class="eyebrow">Historical baseline</p><h3>Pre-season rankings</h3></div><p>These stay based only on Seasons 1–4, preserving the outlook before any Season 5 results were recorded.</p></div><div class="table-shell prediction-table-shell"><table class="profile-table prediction-table"><thead><tr><th>Rank</th><th>Driver</th><th>Pre-season rating</th><th>Championship %</th><th>Title odds</th></tr></thead><tbody>' + preseasonRows.map((row, index) => '<tr><td class="standing-rank ' + (index < 3 ? 'top-three' : '') + '">' + String(index + 1).padStart(2, '0') + '</td><td>' + driverLink(row.name, 'record-driver-link') + '</td><td class="prediction-rating">' + row.preseasonRating.toFixed(1) + '</td><td>' + predictionPercent(row.championshipProbability) + '</td><td>' + predictionAmericanOdds(row.championshipProbability) + '</td></tr>').join('') + '</tbody></table></div></section>' + retired;
  }
  // Teams are deliberately read from one roster source. Season 4 is a
  // retrospective version of the Season 5 roster; later rosters can be added
  // to constructorsTeamsBySeason without changing profile or prediction code.
  const teamVisuals = {
    chr: { initials: 'CHR', primary: '#ff5d36', secondary: '#13171e' },
    jnga: { initials: 'JNGA', primary: '#236ee8', secondary: '#071b3b' },
    'clr-honda': { initials: 'OLR', primary: '#e2b10c', secondary: '#25211a' },
    bbamm: { initials: 'BB', primary: '#8e4de8', secondary: '#21143d' }
  };
  const teamChampionshipPredictionCache = new Map();
  const teamTableDefaults = { name: 'asc', position: 'asc', starts: 'desc', points: 'desc', averagePoints: 'desc', seasonPoints: 'desc', wins: 'desc', podiums: 'desc', poles: 'desc', fastestLaps: 'desc', lapsLed: 'desc', avgFinish: 'asc', avgQualifying: 'asc', bestFinish: 'asc', bestQualifying: 'asc', round: 'asc', teamRank: 'asc', totalPoints: 'desc' };
  function teamVisual(team) { return teamVisuals[team.id] || { initials: team.name.slice(0, 3).toUpperCase(), primary: '#2774e8', secondary: '#12213e' }; }
  function teamLogo(team, compact = false) { const visual = teamVisual(team); return '<span class="team-logo' + (compact ? ' is-compact' : '') + '" style="--team-primary:' + visual.primary + ';--team-secondary:' + visual.secondary + '" aria-hidden="true">' + escapeHtml(visual.initials) + '</span>'; }
  function teamLink(team, className = 'team-link') { return '<button type="button" class="' + className + '" data-team-profile-id="' + escapeHtml(team.id) + '">' + escapeHtml(team.name) + '</button>'; }
  function teamSeasonOptions() { return seasons.filter((season) => constructorsTeamsBySeason[season.id]); }
  function teamSeason() {
    const allowed = teamSeasonOptions();
    if (!allowed.some((season) => season.id === state.teamSeason)) state.teamSeason = constructorsTeamsBySeason[getSeason().id] ? getSeason().id : (allowed[allowed.length - 1]?.id || '5');
    return seasons.find((season) => season.id === state.teamSeason) || allowed[allowed.length - 1];
  }
  function teamSeasonData(season = teamSeason()) { return season ? constructorsSeasonStandings(season) : { definition: null, teams: [] }; }
  function teamById(teamId, season = teamSeason()) { return teamSeasonData(season).teams.find((team) => team.id === teamId) || null; }
  function teamRoundMetric(team, season, round) {
    const entries = team.drivers.map((name) => {
      const driver = season.drivers.find((item) => item.name === name); const result = driver?.results[round.index] || null;
      if (!result || !enhResultHasFinish(result)) return null;
      const bonus = getChampionshipBonus({ ...result, race: round.race, roundIndex: round.index }, season);
      return { name, result, points: (result.points || 0) + bonus.totalBonusPoints };
    }).filter(Boolean);
    const finishes = entries.map((entry) => entry.result.position); const qualifying = entries.map((entry) => entry.result.qualifyingPosition).filter(Number.isFinite);
    const totalPoints = entries.reduce((total, entry) => total + entry.points, 0);
    return {
      round, entries, starters: entries.length, totalPoints,
      averagePoints: entries.length ? totalPoints / entries.length : 0,
      avgFinish: finishes.length ? finishes.reduce((total, value) => total + value, 0) / finishes.length : null,
      avgQualifying: qualifying.length ? qualifying.reduce((total, value) => total + value, 0) / qualifying.length : null,
      lapsLed: entries.reduce((total, entry) => total + (entry.result.lapsLed || 0), 0)
    };
  }
  function teamRoundRankings(season, round) {
    const data = constructorsSeasonStandings(season);
    return data.teams.map((team) => ({ team, ...teamRoundMetric(team, season, round) })).filter((row) => row.starters)
      .sort((first, second) => first.avgFinish - second.avgFinish || second.averagePoints - first.averagePoints || first.team.name.localeCompare(second.team.name))
      .map((row, index) => ({ ...row, teamRank: index + 1 }));
  }
  function teamCombinedStats(teamId, scope = 'career') {
    const selected = scope === 'career' ? teamSeasonOptions() : [seasons.find((season) => season.id === scope)].filter(Boolean);
    const rows = selected.map((season) => ({ season, team: teamById(teamId, season) })).filter((row) => row.team);
    const weighted = (key) => {
      const applicable = rows.filter((row) => Number.isFinite(row.team[key]) && row.team.starts);
      return applicable.length ? applicable.reduce((total, row) => total + row.team[key] * row.team.starts, 0) / applicable.reduce((total, row) => total + row.team.starts, 0) : null;
    };
    return {
      id: teamId, team: rows[0]?.team || null,
      seasons: rows, starts: rows.reduce((total, row) => total + row.team.starts, 0),
      points: rows.reduce((total, row) => total + row.team.totalDriverPoints, 0),
      seasonPoints: rows.reduce((total, row) => total + row.team.seasonPoints, 0),
      scoringRounds: rows.reduce((total, row) => total + row.team.scoringRounds.length, 0),
      averagePoints: (() => { const totals = rows.reduce((total, row) => total + row.team.seasonPoints, 0); const races = rows.reduce((total, row) => total + row.team.scoringRounds.length, 0); return races ? totals / races : null; })(),
      wins: rows.reduce((total, row) => total + row.team.wins, 0), podiums: rows.reduce((total, row) => total + row.team.podiums, 0), poles: rows.reduce((total, row) => total + row.team.poles, 0), fastestLaps: rows.reduce((total, row) => total + row.team.fastestLaps, 0), lapsLed: rows.reduce((total, row) => total + row.team.lapsLed, 0),
      avgFinish: weighted('avgFinish'), avgQualifying: weighted('avgQualifying'),
      bestFinish: Math.min(...rows.map((row) => row.team.bestFinish).filter(Number.isFinite), Infinity),
      bestQualifying: Math.min(...rows.map((row) => row.team.bestQualifying).filter(Number.isFinite), Infinity),
      championships: rows.filter((row) => row.team.position === 1).length
    };
  }
  function teamRosterEntries(driverName) {
    return teamSeasonOptions().flatMap((season) => {
      const definition = constructorsTeamsBySeason[season.id]; const team = definition.teams.find((item) => item.drivers.includes(driverName));
      const driver = season.drivers.find((item) => item.name === driverName);
      if (!team || !driver) return [];
      return getArchiveRounds(season).map((round) => ({ season, team, round, result: driver.results[round.index] || {} })).filter((entry) => enhResultHasFinish(entry.result));
    });
  }
  function driverCurrentTeamCard(name) {
    const season = constructorsTeamsBySeason[getSeason().id] ? getSeason() : teamSeasonOptions()[teamSeasonOptions().length - 1];
    const team = constructorsTeamsBySeason[season?.id]?.teams.find((item) => item.drivers.includes(name));
    if (!team) return '';
    const standing = teamById(team.id, season);
    return '<section class="driver-team-card"><div>' + teamLogo(team, true) + '<div><p class="eyebrow">Current team</p>' + teamLink(team, 'team-link') + '<p>Constructors position ' + (standing ? 'P' + standing.position : '—') + ' · ' + (standing ? constructorsPoints(standing.seasonPoints) + ' constructors points' : 'Team stats pending') + '</p></div></div><span class="team-color-swatches" aria-label="Team colors"><i style="background:' + teamVisual(team).primary + '"></i><i style="background:' + teamVisual(team).secondary + '"></i></span></section>';
  }
  function renderDriverTeamStatistics(name) {
    const entries = teamRosterEntries(name); const stats = getStats(entries.map((entry) => entry.result));
    const teams = [...new Map(entries.map((entry) => [entry.team.id, entry.team])).values()];
    const teamNames = teams.map((team) => teamLink(team, 'team-link')).join(', ') || 'No team roster data';
    const byTeam = teams.map((team) => {
      const results = entries.filter((entry) => entry.team.id === team.id).map((entry) => entry.result); const own = getStats(results); const teamRows = teamSeasonOptions().filter((season) => constructorsTeamsBySeason[season.id].teams.some((item) => item.id === team.id && item.drivers.includes(name))).map((season) => teamById(team.id, season));
      return { team, starts: own.completed.length, wins: own.wins, podiums: own.podiums, poles: own.poles, fastestLaps: own.fastestLaps, avgFinish: own.avgFinish, teamWins: teamRows.reduce((total, row) => total + (row?.wins || 0), 0) };
    });
    return '<section class="profile-panel team-driver-statistics"><div class="panel-title"><div><p class="eyebrow">Constructor connection</p><h3>Team statistics</h3></div><p>Team-based totals only include seasons where this driver was listed on the roster.</p></div><p class="team-roster-note">Current roster: ' + teamNames + '</p><div class="team-stat-grid"><article><strong>' + stats.wins + '</strong><span>Driver wins while rostered</span></article><article><strong>' + stats.podiums + '</strong><span>Driver podiums while rostered</span></article><article><strong>' + stats.poles + '</strong><span>Driver poles while rostered</span></article><article><strong>' + stats.fastestLaps + '</strong><span>Fastest laps while rostered</span></article><article><strong>' + average(stats.avgFinish) + '</strong><span>Average finish while rostered</span></article></div><div class="mini-table-shell"><table class="profile-table"><thead><tr><th>Team</th><th>Starts</th><th>Wins</th><th>Podiums</th><th>Poles</th><th>Fastest laps</th><th>Avg. finish</th><th>Team wins while rostered</th></tr></thead><tbody>' + (byTeam.map((row) => '<tr><td>' + teamLogo(row.team, true) + teamLink(row.team, 'team-link') + '</td><td>' + row.starts + '</td><td>' + row.wins + '</td><td>' + row.podiums + '</td><td>' + row.poles + '</td><td>' + row.fastestLaps + '</td><td>' + average(row.avgFinish) + '</td><td>' + row.teamWins + '</td></tr>').join('') || '<tr><td colspan="8">No rostered team starts are recorded.</td></tr>') + '</tbody></table></div></section>';
  }
  function teamSortHeader(label, key, scope) {
    const current = state.teamSorts?.[scope] || { key: teamTableDefaults[key] ? key : 'name', direction: teamTableDefaults[key] || 'asc' };
    const active = current.key === key;
    return '<th><button class="sort-button" type="button" data-team-sort-scope="' + scope + '" data-team-sort-key="' + key + '" aria-pressed="' + active + '">' + label + ' <span class="sort-icon" aria-hidden="true">' + (active ? (current.direction === 'asc' ? '↑' : '↓') : '↕') + '</span></button></th>';
  }
  function teamSortRows(rows, scope, fallback = 'name') {
    const sort = state.teamSorts?.[scope] || { key: fallback, direction: teamTableDefaults[fallback] || 'asc' };
    return rows.slice().sort((first, second) => {
      const a = first[sort.key]; const b = second[sort.key];
      if (a === null || a === undefined || !Number.isFinite(a) && typeof a !== 'string') return 1;
      if (b === null || b === undefined || !Number.isFinite(b) && typeof b !== 'string') return -1;
      return (typeof a === 'string' ? a.localeCompare(b) : a - b) * (sort.direction === 'asc' ? 1 : -1) || String(first.name || first.driver || '').localeCompare(String(second.name || second.driver || ''));
    });
  }
  function teamMetricCards(team) {
    const card = (label, value) => '<article><strong>' + value + '</strong><span>' + label + '</span></article>';
    return '<div class="team-stat-grid team-season-summary">' + card('Constructors position', 'P' + team.position) + card('Constructors points', constructorsPoints(team.seasonPoints)) + card('Average points', constructorsPoints(team.averagePoints)) + card('Total driver points', number.format(team.totalDriverPoints)) + card('Wins', team.wins) + card('Podiums', team.podiums) + card('Poles', team.poles) + card('Fastest laps', team.fastestLaps) + card('Laps led', team.lapsLed) + card('Average finish', average(team.avgFinish)) + card('Average qualifying', average(team.avgQualifying)) + card('Best finish', position(team.bestFinish)) + card('Best qualifying', position(team.bestQualifying)) + '</div>';
  }
  function renderTeamProgression(season, selectedTeamId) {
    const data = teamSeasonData(season); const rounds = getArchiveRounds(season); const visible = state.teamProgressionMode === 'select' ? data.teams.filter((team) => state.teamProgressionSelected.has(team.id)) : data.teams;
    const lines = visible.length ? visible : data.teams.filter((team) => team.id === selectedTeamId);
    const values = lines.flatMap((team) => { let total = 0; return rounds.map((round) => { total += team.roundAverages.find((row) => row.index === round.index)?.averagePoints || 0; return total; }); });
    if (!rounds.length || !lines.length) return '<p class="no-profile">No completed team rounds are available for a progression graph.</p>';
    const width = 900; const height = 350; const margin = { top: 30, right: 30, bottom: 58, left: 64 }; const maxValue = Math.max(1, ...values); const plotWidth = width - margin.left - margin.right; const plotHeight = height - margin.top - margin.bottom;
    const x = (index) => margin.left + (rounds.length < 2 ? plotWidth / 2 : index / (rounds.length - 1) * plotWidth); const y = (value) => margin.top + plotHeight - value / maxValue * plotHeight;
    const grid = Array.from({ length: 5 }, (_, index) => { const value = maxValue * index / 4; return '<line class="chart-grid" x1="' + margin.left + '" x2="' + (width - margin.right) + '" y1="' + y(value) + '" y2="' + y(value) + '"></line><text class="chart-axis" x="' + (margin.left - 9) + '" y="' + (y(value) + 4) + '" text-anchor="end">' + constructorsPoints(value) + '</text>'; }).join('');
    const svgLines = lines.map((team) => { let cumulative = 0; const pts = rounds.map((round, index) => { cumulative += team.roundAverages.find((row) => row.index === round.index)?.averagePoints || 0; return { x: x(index), y: y(cumulative), value: cumulative, round }; }); const color = teamVisual(team).primary; return '<path class="progression-line team-progression-line" stroke="' + color + '" d="' + enhSvgLine(pts) + '"></path>' + pts.map((point) => '<circle cx="' + point.x + '" cy="' + point.y + '" r="4" fill="' + color + '"><title>' + escapeHtml(team.name + ' · Round ' + (point.round.index + 1) + ': ' + constructorsPoints(point.value) + ' points') + '</title></circle>').join(''); }).join('');
    const labels = rounds.map((round, index) => '<text class="chart-axis" x="' + x(index) + '" y="' + (height - 27) + '" text-anchor="middle">R' + (round.index + 1) + '</text>').join('');
    const legend = data.teams.map((team) => '<button type="button" data-team-progression-driver="' + team.id + '" aria-pressed="' + (state.teamProgressionMode === 'select' && state.teamProgressionSelected.has(team.id)) + '"><i style="background:' + teamVisual(team).primary + '"></i>' + escapeHtml(team.name) + '</button>').join('');
    return '<div class="team-progression-controls"><div class="segmented-controls"><button type="button" data-team-progression-mode="all" aria-pressed="' + (state.teamProgressionMode !== 'select') + '">All Teams</button><button type="button" data-team-progression-mode="select" aria-pressed="' + (state.teamProgressionMode === 'select') + '">Select Teams</button></div><div class="team-progression-legend" aria-label="Toggle team lines">' + legend + '</div></div><div class="chart-scroll team-progression-chart"><svg viewBox="0 0 ' + width + ' ' + height + '" role="img" aria-label="Cumulative constructors points after every round"><line class="chart-axis-line" x1="' + margin.left + '" x2="' + (width - margin.right) + '" y1="' + (height - margin.bottom) + '" y2="' + (height - margin.bottom) + '"></line><line class="chart-axis-line" x1="' + margin.left + '" x2="' + margin.left + '" y1="' + margin.top + '" y2="' + (height - margin.bottom) + '"></line>' + grid + svgLines + labels + '<text class="chart-axis" x="' + (margin.left + plotWidth / 2) + '" y="' + (height - 9) + '" text-anchor="middle">Race rounds</text><text class="chart-axis" transform="translate(16 ' + (margin.top + plotHeight / 2) + ') rotate(-90)" text-anchor="middle">Cumulative constructors points</text></svg></div>';
  }
  function renderTeamProfile(team, season) {
    const data = teamSeasonData(season); const contributors = teamSortRows(team.active.map((member) => ({ name: member.name, driver: member.driver, starts: member.driver.completed.length, points: member.driver.points, averagePoints: member.driver.completed.length ? member.driver.points / member.driver.completed.length : null, wins: member.driver.wins, podiums: member.driver.podiums, poles: member.driver.poles, fastestLaps: member.driver.fastestLaps, lapsLed: member.driver.lapsLed, avgFinish: member.driver.avgFinish, avgQualifying: member.driver.avgQualifying })), 'team-contributions', 'points');
    const rounds = getArchiveRounds(season); const raceRows = teamSortRows(rounds.map((round) => { const metric = teamRoundMetric(team, season, round); const rank = teamRoundRankings(season, round).find((row) => row.team.id === team.id)?.teamRank || null; return { round: round.index + 1, track: enhTrackName(round.race), event: round.race.name || 'TBC', avgFinish: metric.avgFinish, averagePoints: metric.averagePoints, lapsLed: metric.lapsLed, teamRank: rank, entries: metric.entries, seasonIndex: seasons.indexOf(season), roundIndex: getArchiveRounds(season).findIndex((item) => item.index === round.index) }; }).filter((row) => row.entries.length), 'team-race-log', 'round');
    const roster = team.members.map((member) => member.driver ? '<article class="team-roster-card"><div>' + teamLogo(team, true) + '<div><h4>' + driverLink(member.name, 'record-driver-link') + '</h4><p>P' + member.driver.championshipPosition + ' · ' + number.format(member.driver.points) + ' pts</p></div></div><dl><div><dt>Avg finish</dt><dd>' + average(member.driver.avgFinish) + '</dd></div><div><dt>Wins</dt><dd>' + member.driver.wins + '</dd></div><div><dt>Podiums</dt><dd>' + member.driver.podiums + '</dd></div><div><dt>Poles</dt><dd>' + member.driver.poles + '</dd></div><div><dt>Fastest laps</dt><dd>' + member.driver.fastestLaps + '</dd></div></dl></article>' : '<article class="team-roster-card is-inactive"><div>' + teamLogo(team, true) + '<div><h4>' + escapeHtml(member.name) + '</h4><p>Did not compete in ' + escapeHtml(season.name) + '</p></div></div></article>').join('');
    const contributionRows = contributors.map((row) => '<tr><td>' + driverLink(row.name, 'record-driver-link') + '</td><td>' + row.starts + '</td><td>' + number.format(row.points) + '</td><td>' + average(row.averagePoints) + '</td><td>' + row.wins + '</td><td>' + row.podiums + '</td><td>' + row.poles + '</td><td>' + row.fastestLaps + '</td><td>' + row.lapsLed + '</td><td>' + average(row.avgFinish) + '</td><td>' + average(row.avgQualifying) + '</td></tr>').join('');
    const raceLogRows = raceRows.map((row) => '<tr><td>R' + row.round + '</td><td><button type="button" class="profile-jump-link" data-team-race-season-index="' + row.seasonIndex + '" data-team-race-round-index="' + row.roundIndex + '">' + escapeHtml(row.event) + '</button><small>' + escapeHtml(row.track) + '</small></td><td>' + average(row.avgFinish) + '</td><td>' + constructorsPoints(row.averagePoints) + '</td><td>' + row.entries.map((entry) => driverLink(entry.name, 'record-driver-link') + ' ' + position(entry.result.position)).join(' · ') + '</td><td>' + row.entries.map((entry) => driverLink(entry.name, 'record-driver-link') + ' ' + position(entry.result.qualifyingPosition)).join(' · ') + '</td><td>' + (row.lapsLed || '—') + '</td><td>' + (row.teamRank ? 'P' + row.teamRank : '—') + '</td></tr>').join('');
    return '<section class="team-profile"><article class="team-profile-hero" style="--team-primary:' + teamVisual(team).primary + ';--team-secondary:' + teamVisual(team).secondary + '"><div class="team-profile-title">' + teamLogo(team) + '<div><p class="eyebrow">' + (data.definition.kind === 'hypothetical' ? 'Hypothetical constructors profile' : 'Official constructors profile') + '</p><h3>' + escapeHtml(team.name) + '</h3><p>P' + team.position + ' in constructors · ' + constructorsPoints(team.averagePoints) + ' average points · ' + team.drivers.length + ' drivers</p></div></div><div class="team-color-key"><span>Team colors</span><i></i><i></i></div></article><section class="team-roster-section"><div class="panel-title"><div><p class="eyebrow">Current roster</p><h3>Team roster</h3></div><p>Driver names open their complete Driver Profile.</p></div><div class="team-roster-grid">' + roster + '</div></section><section class="team-summary-section"><div class="panel-title"><div><p class="eyebrow">' + escapeHtml(season.name) + '</p><h3>Team season summary</h3></div><p>Constructors points are added from each round’s active-driver average.</p></div>' + teamMetricCards(team) + '</section><section class="profile-panel"><div class="panel-title"><div><p class="eyebrow">Every driver</p><h3>Driver contributions</h3></div><p>Click a heading to sort.</p></div><div class="mini-table-shell"><table class="profile-table team-contributions-table"><thead><tr>' + teamSortHeader('Driver', 'name', 'team-contributions') + teamSortHeader('Starts', 'starts', 'team-contributions') + teamSortHeader('Points', 'points', 'team-contributions') + teamSortHeader('Avg. points', 'averagePoints', 'team-contributions') + teamSortHeader('Wins', 'wins', 'team-contributions') + teamSortHeader('Podiums', 'podiums', 'team-contributions') + teamSortHeader('Poles', 'poles', 'team-contributions') + teamSortHeader('Fastest laps', 'fastestLaps', 'team-contributions') + teamSortHeader('Laps led', 'lapsLed', 'team-contributions') + teamSortHeader('Avg. finish', 'avgFinish', 'team-contributions') + teamSortHeader('Avg. qualifying', 'avgQualifying', 'team-contributions') + '</tr></thead><tbody>' + (contributionRows || '<tr><td colspan="11">No active driver contributions are recorded.</td></tr>') + '</tbody></table></div></section><section class="profile-panel"><div class="panel-title"><div><p class="eyebrow">Round by round</p><h3>Team race results</h3></div><p>Race names jump directly to Race Results. Click a heading to sort.</p></div><div class="mini-table-shell"><table class="profile-table team-race-log"><thead><tr>' + teamSortHeader('Round', 'round', 'team-race-log') + teamSortHeader('Track', 'track', 'team-race-log') + teamSortHeader('Avg. finish', 'avgFinish', 'team-race-log') + teamSortHeader('Avg. points', 'averagePoints', 'team-race-log') + '<th>Individual finishes</th><th>Individual qualifying</th>' + teamSortHeader('Laps led', 'lapsLed', 'team-race-log') + teamSortHeader('Team rank', 'teamRank', 'team-race-log') + '</tr></thead><tbody>' + (raceLogRows || '<tr><td colspan="8">No team race results are recorded.</td></tr>') + '</tbody></table></div></section><section class="profile-panel team-progression-panel"><div class="panel-title"><div><p class="eyebrow">Race-by-race points</p><h3>Constructors Championship Progression</h3></div><p>Lines show cumulative constructors points, not position.</p></div>' + renderTeamProgression(season, team.id) + '</section></section>';
  }
  function teamH2HRaceData(teamA, teamB, scope) {
    const seasonsToUse = scope === 'career' ? teamSeasonOptions() : [seasons.find((season) => season.id === scope)].filter(Boolean); const results = [];
    seasonsToUse.forEach((season) => getArchiveRounds(season).forEach((round) => { const a = teamRoundMetric(teamA, season, round); const b = teamRoundMetric(teamB, season, round); if (a.starters && b.starters) results.push({ season, round, a, b, outcome: a.avgFinish < b.avgFinish ? 'a' : a.avgFinish > b.avgFinish ? 'b' : 'tie' }); }));
    const streak = (key) => { let longest = 0; let current = 0; let start = null; let longestStart = null; let longestEnd = null; results.forEach((row) => { if (row.outcome === key) { if (!current) start = row; current += 1; if (current > longest) { longest = current; longestStart = start; longestEnd = row; } } else current = 0; }); return { length: longest, start: longestStart, end: longestEnd }; };
    const current = (() => { let outcome = results[results.length - 1]?.outcome; if (!outcome || outcome === 'tie') return { outcome: 'tie', length: 0 }; let length = 0; for (let index = results.length - 1; index >= 0 && results[index].outcome === outcome; index -= 1) length += 1; return { outcome, length }; })();
    return { results, aWins: results.filter((row) => row.outcome === 'a').length, bWins: results.filter((row) => row.outcome === 'b').length, ties: results.filter((row) => row.outcome === 'tie').length, aStreak: streak('a'), bStreak: streak('b'), current };
  }
  function teamH2HPeriod(streak) { return streak.length ? escapeHtml(streak.start.season.name + ' R' + (streak.start.round.index + 1) + ' – ' + streak.end.season.name + ' R' + (streak.end.round.index + 1)) : '—'; }
  function renderTeamHeadToHead(season) {
    const data = teamSeasonData(season); const scope = state.teamH2HScope || season.id; if (!data.teams.some((team) => team.id === state.teamH2HA)) state.teamH2HA = data.teams[0]?.id; if (!data.teams.some((team) => team.id === state.teamH2HB) || state.teamH2HB === state.teamH2HA) state.teamH2HB = data.teams.find((team) => team.id !== state.teamH2HA)?.id;
    const first = data.teams.find((team) => team.id === state.teamH2HA); const second = data.teams.find((team) => team.id === state.teamH2HB); if (!first || !second) return '<p class="no-profile">At least two teams are needed for a team comparison.</p>';
    const a = teamCombinedStats(first.id, scope); const b = teamCombinedStats(second.id, scope); const h2h = teamH2HRaceData(first, second, scope); const metricRows = [['Wins', 'wins'], ['Podiums', 'podiums'], ['Poles', 'poles'], ['Fastest laps', 'fastestLaps'], ['Laps led', 'lapsLed'], ['Average finish', 'avgFinish'], ['Average qualifying', 'avgQualifying'], ['Average points', 'averagePoints'], ['Constructors championships', 'championships'], ['Race wins', 'wins'], ['Best finish', 'bestFinish'], ['Best qualifying', 'bestQualifying']]; const lowWins = new Set(['avgFinish', 'avgQualifying', 'bestFinish', 'bestQualifying']);
    const comparison = metricRows.map(([label, key]) => { const left = a[key] === Infinity ? null : a[key]; const right = b[key] === Infinity ? null : b[key]; const leftLead = Number.isFinite(left) && Number.isFinite(right) && (lowWins.has(key) ? left < right : left > right); const rightLead = Number.isFinite(left) && Number.isFinite(right) && (lowWins.has(key) ? right < left : right > left); const display = (value, metric) => ['avgFinish', 'avgQualifying', 'averagePoints'].includes(metric) ? average(value) : ['bestFinish', 'bestQualifying'].includes(metric) ? position(value) : number.format(value || 0); return '<tr><th scope="row">' + label + '</th><td class="' + (leftLead ? 'team-leading-cell' : '') + '">' + display(left, key) + '</td><td class="' + (rightLead ? 'team-leading-cell' : '') + '">' + display(right, key) + '</td></tr>'; }).join('');
    const options = data.teams.map((team) => '<option value="' + team.id + '">' + escapeHtml(team.name) + '</option>').join('');
    return '<section class="team-h2h-panel"><div class="team-comparison-controls"><label><span>Compare</span><select data-team-h2h="a">' + options.replace('value="' + first.id + '"', 'value="' + first.id + '" selected') + '</select></label><label><span>With</span><select data-team-h2h="b">' + options.replace('value="' + second.id + '"', 'value="' + second.id + '" selected') + '</select></label><label><span>View</span><select data-team-h2h-scope><option value="career"' + (scope === 'career' ? ' selected' : '') + '>Career</option>' + teamSeasonOptions().map((item) => '<option value="' + item.id + '"' + (scope === item.id ? ' selected' : '') + '>' + escapeHtml(item.name) + '</option>').join('') + '</select></label></div><div class="team-comparison-header"><div>' + teamLogo(first, true) + teamLink(first) + '</div><span>VS</span><div>' + teamLogo(second, true) + teamLink(second) + '</div></div><div class="mini-table-shell"><table class="profile-table team-h2h-table"><thead><tr><th>Statistic</th><th>' + escapeHtml(first.name) + '</th><th>' + escapeHtml(second.name) + '</th></tr></thead><tbody>' + comparison + '</tbody></table></div><div class="team-h2h-results"><article><span>Race Head-to-Head</span><strong>' + escapeHtml(first.name) + ' ' + h2h.aWins + '–' + h2h.bWins + ' ' + escapeHtml(second.name) + (h2h.ties ? ' (' + h2h.ties + ' ties)' : '') + '</strong><small>Lower team average finishing position wins each shared round.</small></article><article><span>Largest winning streak</span><strong>' + escapeHtml(first.name) + ': ' + h2h.aStreak.length + ' · ' + escapeHtml(second.name) + ': ' + h2h.bStreak.length + '</strong><small>' + teamH2HPeriod(h2h.aStreak) + ' · ' + teamH2HPeriod(h2h.bStreak) + '</small></article><article><span>Current streak</span><strong>' + (h2h.current.outcome === 'tie' ? 'No active streak' : escapeHtml(h2h.current.outcome === 'a' ? first.name : second.name) + ' · ' + h2h.current.length) + '</strong><small>Only races both teams started are counted.</small></article></div></section>';
  }
  function teamLongestSeasonStreak(team, season, predicate) { let best = 0; let current = 0; getArchiveRounds(season).forEach((round) => { const metric = teamRoundMetric(team, season, round); if (metric.starters && predicate(metric)) { current += 1; best = Math.max(best, current); } else current = 0; }); return best; }
  function renderTeamRecords(season) {
    const careerRows = teamSeasonData(season).teams.map((team) => ({ ...teamCombinedStats(team.id, 'career'), name: team.name })); const seasonRows = teamSeasonOptions().flatMap((item) => teamSeasonData(item).teams.map((team) => ({ ...team, name: team.name, season: item.name, consecutiveWins: teamLongestSeasonStreak(team, item, (metric) => metric.entries.some((entry) => entry.result.position === 1)), consecutivePodiums: teamLongestSeasonStreak(team, item, (metric) => metric.entries.some((entry) => entry.result.position <= 3)) }))); const raceRows = teamSeasonOptions().flatMap((item) => getArchiveRounds(item).flatMap((round) => teamRoundRankings(item, round).map((row, index, all) => ({ name: row.team.name, team: row.team, season: item.name, round: round.index + 1, lapsLed: row.lapsLed, avgFinish: row.avgFinish, topFive: row.entries.filter((entry) => entry.result.position <= 5).length, topTen: row.entries.filter((entry) => entry.result.position <= 10).length, margin: index === 0 && all[1] ? all[1].avgFinish - row.avgFinish : null }))));
    const table = (scope, title, rows, columns, fallback) => '<section class="profile-panel team-record-panel"><div class="panel-title"><div><p class="eyebrow">Team records</p><h3>' + title + '</h3></div><p>Click a heading to sort.</p></div><div class="mini-table-shell"><table class="profile-table"><thead><tr>' + columns.map((column) => teamSortHeader(column.label, column.key, scope)).join('') + '</tr></thead><tbody>' + teamSortRows(rows, scope, fallback).map((row) => '<tr>' + columns.map((column) => '<td>' + (column.render ? column.render(row) : escapeHtml(row[column.key] ?? '—')) + '</td>').join('') + '</tr>').join('') + '</tbody></table></div></section>';
    const name = (row) => row.team ? teamLink(row.team, 'team-link') : escapeHtml(row.name); const value = (key) => (row) => key.includes('avg') ? average(row[key]) : key === 'seasonPoints' || key === 'averagePoints' ? constructorsPoints(row[key]) : number.format(row[key] || 0);
    return '<div class="team-records-grid">' + table('team-career-records', 'Career records', careerRows, [{ label: 'Team', key: 'name', render: name }, { label: 'Championships', key: 'championships', render: value('championships') }, { label: 'Wins', key: 'wins', render: value('wins') }, { label: 'Podiums', key: 'podiums', render: value('podiums') }, { label: 'Poles', key: 'poles', render: value('poles') }, { label: 'Fastest laps', key: 'fastestLaps', render: value('fastestLaps') }, { label: 'Laps led', key: 'lapsLed', render: value('lapsLed') }, { label: 'Avg. finish', key: 'avgFinish', render: value('avgFinish') }, { label: 'Avg. qualifying', key: 'avgQualifying', render: value('avgQualifying') }], 'wins') + table('team-season-records', 'Season records', seasonRows, [{ label: 'Team', key: 'name', render: name }, { label: 'Season', key: 'season' }, { label: 'Constructors points', key: 'seasonPoints', render: value('seasonPoints') }, { label: 'Wins', key: 'wins', render: value('wins') }, { label: 'Podiums', key: 'podiums', render: value('podiums') }, { label: 'Poles', key: 'poles', render: value('poles') }, { label: 'Fastest laps', key: 'fastestLaps', render: value('fastestLaps') }, { label: 'Laps led', key: 'lapsLed', render: value('lapsLed') }, { label: 'Avg. finish', key: 'avgFinish', render: value('avgFinish') }, { label: 'Consecutive wins', key: 'consecutiveWins', render: value('consecutiveWins') }, { label: 'Consecutive podiums', key: 'consecutivePodiums', render: value('consecutivePodiums') }], 'seasonPoints') + table('team-race-records', 'Race records', raceRows, [{ label: 'Team', key: 'name', render: name }, { label: 'Season', key: 'season' }, { label: 'Round', key: 'round' }, { label: 'Margin over second', key: 'margin', render: (row) => Number.isFinite(row.margin) ? row.margin.toFixed(2) : '—' }, { label: 'Laps led', key: 'lapsLed', render: value('lapsLed') }, { label: 'Avg. finish', key: 'avgFinish', render: value('avgFinish') }, { label: 'Drivers in Top 5', key: 'topFive', render: value('topFive') }, { label: 'Drivers in Top 10', key: 'topTen', render: value('topTen') }], 'margin') + '</div>';
  }
  function renderTeams() {
    const season = teamSeason(); const data = teamSeasonData(season); const container = elements.teamsContent; if (!container || !data.definition) return;
    if (!data.teams.some((team) => team.id === state.selectedTeamId)) state.selectedTeamId = data.teams[0]?.id || '';
    const view = state.teamsView || 'profile'; const selected = data.teams.find((team) => team.id === state.selectedTeamId); const controls = '<div class="teams-controls"><div class="segmented-controls" aria-label="Teams views"><button type="button" data-teams-view="profile" aria-pressed="' + (view === 'profile') + '">Team Profile</button><button type="button" data-teams-view="head-to-head" aria-pressed="' + (view === 'head-to-head') + '">Team vs Team</button><button type="button" data-teams-view="records" aria-pressed="' + (view === 'records') + '">Team Records</button></div><label><span>Season</span><select data-team-season>' + teamSeasonOptions().map((item) => '<option value="' + item.id + '"' + (item.id === season.id ? ' selected' : '') + '>' + escapeHtml(item.name) + (constructorsTeamsBySeason[item.id].kind === 'hypothetical' ? ' (hypothetical)' : '') + '</option>').join('') + '</select></label></div>';
    const chooser = '<div class="team-chooser" aria-label="Choose a team">' + data.teams.map((team) => '<button type="button" data-team-select="' + team.id + '" aria-pressed="' + (team.id === selected?.id) + '">' + teamLogo(team, true) + '<span>' + escapeHtml(team.name) + '</span><small>P' + team.position + ' · ' + constructorsPoints(team.seasonPoints) + ' pts</small></button>').join('') + '</div>';
    const body = view === 'head-to-head' ? renderTeamHeadToHead(season) : view === 'records' ? renderTeamRecords(season) : selected ? renderTeamProfile(selected, season) : '<p class="no-profile">No team data is available.</p>';
    container.innerHTML = controls + (view === 'profile' ? chooser : '') + body;
  }
  function openTeam(teamId, seasonId = getSeason().id, scroll = true) {
    state.teamSeason = constructorsTeamsBySeason[seasonId] ? seasonId : teamSeasonOptions()[teamSeasonOptions().length - 1]?.id;
    state.selectedTeamId = teamId; state.teamsView = 'profile'; renderTeams();
    if (scroll) document.querySelector('#teams')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }
  function setupTeams() {
    Object.assign(state, { teamsView: 'profile', teamSeason: constructorsTeamsBySeason[getSeason().id] ? getSeason().id : '5', selectedTeamId: '', teamSorts: {}, teamProgressionMode: 'all', teamProgressionSelected: new Set(), teamH2HA: '', teamH2HB: '', teamH2HScope: '' });
    elements.teamsContent = document.querySelector('#teams-content');
    document.addEventListener('click', (event) => { const team = event.target.closest('[data-team-profile-id]'); if (team) { openTeam(team.dataset.teamProfileId); } });
    elements.teamsContent?.addEventListener('click', (event) => {
      const view = event.target.closest('[data-teams-view]'); const select = event.target.closest('[data-team-select]'); const sort = event.target.closest('[data-team-sort-scope]'); const mode = event.target.closest('[data-team-progression-mode]'); const line = event.target.closest('[data-team-progression-driver]'); const race = event.target.closest('[data-team-race-season-index]');
      if (view) { state.teamsView = view.dataset.teamsView; renderTeams(); return; }
      if (select) { state.selectedTeamId = select.dataset.teamSelect; renderTeams(); return; }
      if (sort) { const scope = sort.dataset.teamSortScope; const key = sort.dataset.teamSortKey; const current = state.teamSorts[scope]; state.teamSorts[scope] = current?.key === key ? { key, direction: current.direction === 'asc' ? 'desc' : 'asc' } : { key, direction: teamTableDefaults[key] || 'asc' }; renderTeams(); return; }
      if (mode) { state.teamProgressionMode = mode.dataset.teamProgressionMode; if (state.teamProgressionMode === 'select' && !state.teamProgressionSelected.size && state.selectedTeamId) state.teamProgressionSelected.add(state.selectedTeamId); renderTeams(); return; }
      if (line) { state.teamProgressionMode = 'select'; const id = line.dataset.teamProgressionDriver; state.teamProgressionSelected.has(id) ? state.teamProgressionSelected.delete(id) : state.teamProgressionSelected.add(id); renderTeams(); return; }
      if (race) { state.seasonIndex = Number(race.dataset.teamRaceSeasonIndex); state.roundIndex = Number(race.dataset.teamRaceRoundIndex); renderSeason(); document.querySelector('#results')?.scrollIntoView({ behavior: 'smooth', block: 'start' }); }
    });
    elements.teamsContent?.addEventListener('change', (event) => { if (event.target.matches('[data-team-season]')) { state.teamSeason = event.target.value; state.teamH2HScope = event.target.value; renderTeams(); } if (event.target.matches('[data-team-h2h]')) { if (event.target.dataset.teamH2h === 'a') state.teamH2HA = event.target.value; else state.teamH2HB = event.target.value; renderTeams(); } if (event.target.matches('[data-team-h2h-scope]')) { state.teamH2HScope = event.target.value; renderTeams(); } });
  }
  function teamRacePrediction(season, round) {
    const data = predictionRaceSimulationData(season, round); const teams = constructorsTeamsBySeason[season.id]?.teams || []; const tallies = teams.map(() => ({ win: 0, podium: 0, finish: 0, points: 0, count: 0 }));
    data.finishingOrders.forEach((order, simulation) => {
      const scored = teams.map((team, teamIndex) => { const members = team.drivers.map((name) => data.rows.findIndex((row) => row.name === name)).filter((index) => index >= 0); const positions = members.map((index) => order.indexOf(index) + 1).filter((position) => position > 0); const pole = data.rows[data.poleWinners[simulation]]?.name; const fastest = data.rows[data.fastestLapWinners?.[simulation]]?.name; const points = members.reduce((total, index) => { const name = data.rows[index].name; return total + (pointsSystem[order.indexOf(index) + 1] || 0) + (name === pole ? 1 : 0) + (name === fastest ? 1 : 0); }, 0); return { team, teamIndex, averageFinish: positions.length ? positions.reduce((total, value) => total + value, 0) / positions.length : Infinity, averagePoints: members.length ? points / members.length : 0 }; }).filter((row) => Number.isFinite(row.averageFinish)).sort((a, b) => a.averageFinish - b.averageFinish || b.averagePoints - a.averagePoints);
      scored.forEach((row, rank) => { const tally = tallies[row.teamIndex]; tally.count += 1; tally.finish += row.averageFinish; tally.points += row.averagePoints; tally.win += rank === 0 ? 1 : 0; tally.podium += rank < 3 ? 1 : 0; });
    });
    return teams.map((team, index) => { const tally = tallies[index]; return { team, name: team.name, winProbability: tally.count ? tally.win / tally.count : 0, podiumProbability: tally.count ? tally.podium / tally.count : 0, expectedAvgFinish: tally.count ? tally.finish / tally.count : null, expectedAveragePoints: tally.count ? tally.points / tally.count : null }; }).sort((a, b) => a.expectedAvgFinish - b.expectedAvgFinish || b.winProbability - a.winProbability);
  }
  function renderTeamRacePredictions(season, round) { const rows = teamRacePrediction(season, round); return '<section class="prediction-panel team-predictions-panel"><div class="panel-title"><div><p class="eyebrow">Constructors forecast</p><h3>Team Predictions</h3></div><p>Calculated from the exact driver-level simulations above, then scored as an average among each team’s entered drivers.</p></div><div class="table-shell"><table class="profile-table team-predictions-table"><thead><tr><th>Predicted order</th><th>Team</th><th>Win probability</th><th>Podium probability</th><th>Expected avg. finish</th><th>Expected avg. points</th></tr></thead><tbody>' + rows.map((row, index) => '<tr><td>P' + (index + 1) + '</td><td>' + teamLogo(row.team, true) + teamLink(row.team, 'team-link') + '</td><td>' + predictionPercent(row.winProbability) + '</td><td>' + predictionPercent(row.podiumProbability) + '</td><td>' + average(row.expectedAvgFinish) + '</td><td>' + constructorsPoints(row.expectedAveragePoints) + '</td></tr>').join('') + '</tbody></table></div></section>'; }
  function teamChampionshipPrediction(season) {
    const teams = constructorsTeamsBySeason[season.id]?.teams || []; const rounds = getScheduleRounds(season).filter((round) => !predictionCompletedRound(season, round)); const forecasts = rounds.map((round) => ({ round, rows: predictionRaceRows(season, round) })); const starting = constructorsSeasonStandings(season).teams; const cacheKey = [season.id, ...forecasts.flatMap(({ round, rows }) => rows.map((row) => round.index + ':' + row.name + ':' + row.rating.toFixed(5))), ...starting.map((team) => team.id + ':' + team.seasonPoints.toFixed(6))].join('|'); if (teamChampionshipPredictionCache.has(cacheKey)) return teamChampionshipPredictionCache.get(cacheKey); const tallies = teams.map(() => ({ champion: 0, position: 0, points: 0 })); const simulations = 750; const seed = 'team-championship|' + cacheKey; const random = predictionRandom(seed);
    for (let simulation = 0; simulation < simulations; simulation += 1) { const totals = Object.fromEntries(teams.map((team) => [team.id, starting.find((row) => row.id === team.id)?.seasonPoints || 0])); forecasts.forEach(({ rows: forecast }) => { const sampled = predictionSimulationRows(forecast, random); const order = predictionDrawOrder(sampled, random); const pole = sampled[predictionPick(sampled.map((_, index) => index), sampled, random, 'poleWeight')]?.name; const fastest = sampled[predictionPick(sampled.map((_, index) => index), sampled, random, 'fastestLapWeight')]?.name; teams.forEach((team) => { const members = team.drivers.map((name) => forecast.findIndex((row) => row.name === name)).filter((index) => index >= 0); const points = members.reduce((sum, index) => { const name = forecast[index].name; return sum + (pointsSystem[order.indexOf(index) + 1] || 0) + (name === pole ? 1 : 0) + (name === fastest ? 1 : 0); }, 0); if (members.length) totals[team.id] += points / members.length; }); }); const order = teams.slice().sort((a, b) => totals[b.id] - totals[a.id] || a.name.localeCompare(b.name)); order.forEach((team, index) => { tallies[teams.indexOf(team)].champion += index === 0 ? 1 : 0; tallies[teams.indexOf(team)].position += index + 1; tallies[teams.indexOf(team)].points += totals[team.id]; }); }
    const forecast = teams.map((team, index) => ({ team, championshipProbability: tallies[index].champion / simulations, expectedPosition: tallies[index].position / simulations, expectedPoints: tallies[index].points / simulations })).sort((a, b) => b.championshipProbability - a.championshipProbability || a.expectedPosition - b.expectedPosition);
    teamChampionshipPredictionCache.set(cacheKey, forecast); return forecast;
  }
  function renderTeamChampionshipPredictions(season) { const rows = teamChampionshipPrediction(season); return '<section class="prediction-panel team-predictions-panel"><div class="panel-title"><div><p class="eyebrow">Constructors forecast</p><h3>Constructors Championship Probability</h3></div><p>Every remaining race is simulated with its own driver-level event ratings before team points are averaged.</p></div><div class="table-shell"><table class="profile-table team-predictions-table"><thead><tr><th>Team</th><th>Championship probability</th><th>Expected finishing position</th><th>Expected average points</th></tr></thead><tbody>' + rows.map((row) => '<tr><td>' + teamLogo(row.team, true) + teamLink(row.team, 'team-link') + '</td><td>' + predictionPercent(row.championshipProbability) + '</td><td>' + row.expectedPosition.toFixed(2) + '</td><td>' + constructorsPoints(row.expectedPoints) + '</td></tr>').join('') + '</tbody></table></div></section>'; }
  const renderSeasonPredictionBase = renderSeason;
  renderSeason = function renderSeasonWithPredictions() { renderSeasonPredictionBase(); renderPredictions(); renderTeams(); };
  ...
  Object.assign(state, {
    ...
    predictionRaceView: 'full', predictionH2HMode: 'all', predictionH2HAllSortKey: 'closeness', predictionH2HAllSortDirection: 'asc', predictionH2HDriverSortKey: 'closeness', predictionH2HDriverSortDirection: 'asc', predictionH2HDriver: '', predictionH2HFilterDriver: '', predictionH2HVisible: 25
  });
  ...
  elements.predictionsContent?.addEventListener('click', (event) => {
    const mode = event.target.closest('[data-prediction-mode]'); const raceView = event.target.closest('[data-prediction-race-view]'); const headToHeadMode = event.target.closest('[data-prediction-h2h-mode]'); const headToHeadSort = event.target.closest('[data-prediction-h2h-sort-key]'); const showMore = event.target.closest('[data-prediction-h2h-more]'); const sort = event.target.closest('[data-prediction-sort-key]');
    ...
    if (headToHeadSort) {
      const key = headToHeadSort.dataset.predictionH2hSortKey; const isAll = state.predictionH2HMode === 'all'; const currentKey = isAll ? state.predictionH2HAllSortKey : state.predictionH2HDriverSortKey; const currentDirection = isAll ? state.predictionH2HAllSortDirection : state.predictionH2HDriverSortDirection;
      const direction = currentKey === key ? (currentDirection === 'asc' ? 'desc' : 'asc') : predictionH2HDefaultDirection(key);
      if (isAll) { state.predictionH2HAllSortKey = key; state.predictionH2HAllSortDirection = direction; state.predictionH2HVisible = 25; } else { state.predictionH2HDriverSortKey = key; state.predictionH2HDriverSortDirection = direction; }
  ...
  elements.predictionsContent?.addEventListener('change', (event) => {
    ...
    if (event.target.matches('[data-prediction-round-select]')) {
      state.predictionRoundIndex = Number(event.target.value); state.predictionSortKey = 'winProbability'; state.predictionSortDirection = 'desc'; state.predictionH2HAllSortKey = 'closeness'; state.predictionH2HAllSortDirection = 'asc'; state.predictionH2HDriverSortKey = 'closeness'; state.predictionH2HDriverSortDirection = 'asc'; state.predictionH2HFilterDriver = ''; state.predictionH2HVisible = 25;
      const eligible = predictionDrivers(season); if (!eligible.includes(state.predictionH2HDriver)) state.predictionH2HDriver = '';
      renderPredictions(); return;
    }
    if (event.target.matches('[data-prediction-h2h-filter-driver]')) { state.predictionH2HFilterDriver = event.target.value; state.predictionH2HVisible = 25; renderPredictions(); return; }
    if (event.target.matches('[data-prediction-h2h-sort-select]')) {
      const key = event.target.value; const isAll = state.predictionH2HMode === 'all';
      if (isAll) { state.predictionH2HAllSortKey = key; state.predictionH2HAllSortDirection = predictionH2HDefaultDirection(key); state.predictionH2HVisible = 25; } else { state.predictionH2HDriverSortKey = key; state.predictionH2HDriverSortDirection = predictionH2HDefaultDirection(key); }
      renderPredictions(); return;
    }
  ...
  window.GTO_LEAGUE = {
    ...
    teamRacePrediction,
    teamChampionshipPrediction,
    constructorsSeasonStandings,
    constructorsTeamsBySeason,
